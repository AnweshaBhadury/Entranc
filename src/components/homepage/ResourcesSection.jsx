// src/components/ResourcesSection/ResourcesSection.jsx
...
// Fallback cards (used if Sanity has no data)
const fallbackCards = [
  {
    type: "content",
...
const ResourcesSection = () => {
  ...
  const heading = sectionData?.heading || "Community & Resources";
  const cards = sectionData?.cards?.length ? sectionData.cards : fallbackCards;
            ...
            <ScrollReveal key={index} delay={(index % 3) * 0.1}>
              <div
                className={`rounded-2xl overflow-hidden h-64 ${
                  card.type === "content"
                    ? `${card.containerClass || "bg-s2 text-primary"} p-8 flex flex-col justify-between`
                    : ""
                }`}
              >
                {card.type === "image" ? (
                  <img
                    ...
                    alt={card.image?.alt || "Community Resource"}
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <>
                    <div>
                      <h3 className="text-2xl font-bold">{card.title || "Title Placeholder"}</h3>
                      <p className="mt-2 text-sm">{card.description || "Description placeholder text"}</p>
                    </div>
                    <button
                      className={`mt-6 font-bold py-3 px-6 rounded-lg self-start w-full text-center transition-colors ${
                        card.buttonClass || "bg-white text-primary"
                      }`}
                    >
